...
tags: [pentesting, exploit, fuzz, sqlmap, webshell, file upload vuln]
...
### File Uploading Webshell

Now that we're inside using the admin account, we can check the contents of the panel. Upon inspection, we noticed an interesting folder named 'uploads'. Since we have admin privileges, we can upload a file. Let's try uploading a webshell with the next PHP code.
...
this [exploit](https://rayhan0x01.github.io/ctf/2021/04/02/blind-sqli-over-websocket-automation.html)
