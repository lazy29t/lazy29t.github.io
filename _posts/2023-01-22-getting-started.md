...
file.pdf 
```
 We will use `exiftool` to know the metadata of tha file
...
It is discovered that file is generate by `pdfkit`, so with that we are going reasearch about this [version](https://security.snyk.io/vuln/SNYK-RUBY-PDFKIT-2869795) and discover some exploit.
